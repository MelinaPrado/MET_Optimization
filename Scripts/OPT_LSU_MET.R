...
# Loading files
data <- read.csv("PC_optim_output_single-trials.csv")
...
fitMET_EC <- mmer(BLUE ~ Year,
                  random= ~ vsr(Location, Gu=Ekinship) + genotype + genotype:Location,
                  weights = w,
                  rcov = ~ units,
                  data = data, 
                  verbose = FALSE)
...
replicates <- 10
grid2 <- expand.grid(unique(data$cluster), 1:replicates)

system.time(
  OPT_MET <- foreach(i = 1:nrow(grid2), 
...
# Cost per scenario
final_tab$Cost <- c(56.25,56.25,56.25,22.5) 
