...
grid1 <- expand.grid(trials, unique(data$studyYear)) #Just 2021 and 2022 
...
system.time( #time to execute the code
  results.st <- foreach(i = 1:nrow(grid1), #iterate over each row of grid1
    ...
    sample <- droplevels.data.frame(data[data$Location == grid1[i,1] & data$studyYear == grid1[i,2],])
    ...
    output <- data.frame(blues,
                         ...
                         Year = as.character(unique(sample$studyYear)),
                         H.cullis = H2.Cullis,
...
fitMET <- mmer(BLUE ~ Location,
               random= ~ genotype + genotype:Location,
               weights = w, #Study weights
               rcov = ~ units,
               data = results.st,
               verbose = FALSE)

# predicting the BLUP for each location
BLUEs.e <- predict.mmer(object = fitMET, D = "Location")$pvals
