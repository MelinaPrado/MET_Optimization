...
This repository contains the project related to the manuscript "Optimizing multi-environment trials in the US Rice belt via smart-climate-soil prediction based-models and economic importance" by Melina Prado, Adam Famoso, Kurt Guidry, and Roberto Fritsche-Neto (2024).
...
### LSU_MET.R
This script performs the single trial analysis using linear mixed models to calculate grain yield BLUEs (Best Linear Unbiased Estimators) for each individual trial (location x year). It also selects the environmental covariates that cause the most significant changes in rice yield in the US Rice belt and uses these covariates to divide the LSU experimental sites into five major clusters.
